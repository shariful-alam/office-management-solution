class Manage::UsersController < ApplicationController
  ...
  def show_all_pending
    @all_pending_expenses =  Expense.Pending.order('expenses.id ASC').paginate(:page => params[:pending_expenses], :per_page => 20)
    @all_pending_leaves =  Leafe.Pending.order('leaves.id ASC').paginate(:page => params[:pending_leaves], :per_page => 20)
    @all_pending_incomes =  Income.Pending.order('incomes.id ASC').paginate(:page => params[:pending_incomes], :per_page => 20)
  end
